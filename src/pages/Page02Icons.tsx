...
const ICONS: Record<string, string> = {
  ...
  "eye-off": "M17.94 17.94A10.07 10.07 0 0 1 12 20c-7 0-11-8-11-8a18.45 18.45 0 0 1 5.06-5.94 M9.9 4.24A9.12 9.12 0 0 1 12 4c7 0 11 8 11 8a18.5 18.5 0 0 1-2.16 3.19m-6.72-1.07a3 3 0 1 1-4.24-4.24 M1 1l22 22",
  ...
  phone: "M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07A19.5 19.5 0 0 1 4.72 13 19.79 19.79 0 0 1 1.65 4.35 2 2 0 0 1 3.62 2h3a2 2 0 0 1 2 1.72 12.84 12.84 0 0 0 .7 2.81 2 2 0 0 1-.45 2.11L8.09 9.91A16 16 0 0 0 13 14.85l1.27-1.27a2 2 0 0 1 2.11-.45 12.84 12.84 0 0 0 2.81.7A2 2 0 0 1 21 16.92z",
  "map-pin": "M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z M12 7a3 3 0 1 0 0 6 3 3 0 0 0 0-6z",
  database: "M12 2C6.48 2 2 4.48 2 8s4.48 6 10 6 10-2.68 10-6-4.48-6-10-6z M2 8v4c0 3.32 4.48 6 10 6s10-2.68 10-6V8 M2 12v4c0 3.32 4.48 6 10 6s10-2.68 10-6v-4",
  "trending-up": "M23 6l-9.5 9.5-5-5L1 18 M17 6h6v6",
  award: "M12 15a7 7 0 1 0 0-14 7 7 0 0 0 0 14z M8.21 13.89L7 23l5-3 5 3-1.21-9.12",
};

const CATEGORIES = {
  "Điều hướng": ["home", "menu", "chevron-right", "chevron-down", "chevron-left", "grid", "layers"],
  "Hành động": ["plus", "minus", "edit", "trash", "copy", "refresh-cw", "send", "download", "upload"],
  "Trạng thái": ["check", "check-circle", "x-circle", "alert-triangle", "info", "eye", "eye-off", "lock"],
  "Thông tin": ["user", "users", "bell", "mail", "phone", "map-pin", "tag"],
  "Học vụ": ["book-open", "calendar", "clock", "file-text", "award", "bar-chart", "pie-chart", "trending-up"],
  ...
  return (
    <div style={{ padding: 32, maxWidth: 1100, margin: "0 auto" }}>
      <div style={{ marginBottom: 28 }}>
        <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: "0.1em", textTransform: "uppercase", color: "#7a90a8", marginBottom: 6 }}>02</div>
        <h1 style={{ fontSize: 28, fontWeight: 800, color: "#0f1b2d", margin: 0, marginBottom: 6 }}>Icon System</h1>
        <p style={{ fontSize: 14, color: "#7a90a8", margin: 0 }}>Bộ icon SVG stroke 2px • Feather Icons style • 24×24 viewBox</p>
      </div>

      {/* Controls */}
      <div style={{ display: "flex", gap: 16, marginBottom: 28, alignItems: "center" }}>
        <div style={{ position: "relative", flex: 1, maxWidth: 320 }}>
          <span style={{ position: "absolute", left: 10, top: "50%", transform: "translateY(-50%)", color: "#7a90a8" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          ...
          {[16, 20, 24, 32].map(s => (
            <button
              key={s}
              onClick={() => setSize(s)}
              className={`ems-btn ems-btn-xs ${size === s ? "ems-btn-primary" : "ems-btn-secondary"}`}
            >{s}px</button>
          ))}
        </div>
      </div>
      ...
      {Object.entries(CATEGORIES).map(([cat, iconNames]) => {
        const visible = iconNames.filter(n => !filtered || n.includes(filtered));
              ...
              {visible.map(name => (
                <div
                  ...
                  style={{
                    ...
                    gap: 8, padding: "12px 14px", borderRadius: 8,
                    background: copied === name ? "#e6f4ff" : "white",
                    border: `1px solid ${copied === name ? "#91caff" : "#d1dbe8"}`,
                    cursor: "pointer", transition: "all 0.15s", minWidth: 80,
                  }}
                  onMouseEnter={e => { if (copied !== name) e.currentTarget.style.borderColor = "#4096ff"; }}
                  onMouseLeave={e => { if (copied !== name) e.currentTarget.style.borderColor = "#d1dbe8"; }}
                >
                  <Icon path={ICONS[name]} size={size} color={copied === name ? "#1677ff" : "#2d4a78"} />
                  <span style={{ fontSize: 11, color: "#7a90a8", textAlign: "center", lineHeight: 1.3 }}>{name}</span>
                </div>
              ))}
            </div>
          </div>
      ...
      <div className="page-section">
        ...
        <div style={{ display: "flex", gap: 16, flexWrap: "wrap" }}>
          <button className="ems-btn ems-btn-primary ems-btn-md" style={{ display: "flex", alignItems: "center", gap: 8 }}>
            <Icon path={ICONS.plus} size={15} color="white" /> Thêm sinh viên
          </button>
          <button className="ems-btn ems-btn-secondary ems-btn-md" style={{ display: "flex", alignItems: "center", gap: 8 }}>
            <Icon path={ICONS.download} size={15} color="#4a5e78" /> Xuất Excel
          </button>
          <button className="ems-btn ems-btn-danger ems-btn-md" style={{ display: "flex", alignItems: "center", gap: 8 }}>
            <Icon path={ICONS.trash} size={15} color="white" /> Xóa bản ghi
          </button>
          <div className="ems-badge ems-badge-green" style={{ display: "flex", alignItems: "center", gap: 6 }}>
            <Icon path={ICONS["check-circle"]} size={12} color="#389e0d" /> Đã duyệt
          </div>
          <div className="ems-badge ems-badge-orange" style={{ display: "flex", alignItems: "center", gap: 6 }}>
            <Icon path={ICONS.clock} size={12} color="#d46b08" /> Chờ xử lý
          </div>
          <div className="ems-badge ems-badge-red" style={{ display: "flex", alignItems: "center", gap: 6 }}>
