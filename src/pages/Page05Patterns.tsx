      ...
      <div style={{ display: "flex", gap: 16, alignItems: "flex-start" }}>
        ...
        {showFilter && (
          <div className="ems-card" style={{ width: 280, flexShrink: 0 }}>
            ...
            >
              <div>
                <label className="ems-label" htmlFor="filter-semester">
                  Học kỳ
                </label>
                <select
                  id="filter-semester"
              ...
              <div>
                ...
                {["CNTT", "Kinh tế", "Ngoại ngữ", "Tài chính", "Kỹ thuật"].map(
                  (d) => (
                    <label
                      key={d}
                      style={{
                    ...
                    >
                      <input
                        ...
                        onChange={() =>
                          setFilters((f) => ({
                            ...f,
                            dept: (f.dept as string[]).includes(d)
                              ? (f.dept as string[]).filter(
                                  (x: string) => x !== d,
                                )
                              : [...f.dept as string[], d],
                          }))
                  ...
                  >
                    <input
                      ...
                      onChange={() =>
                        setFilters((f) => ({
                          ...f,
                          status: (f.status as string[]).includes(s)
                            ? (f.status as string[]).filter(
                                (x: string) => x !== s,
                              )
                            : [...f.status as string[], s],
                        }))
                      }
                    />{" "}
                    {s}
              ...
              <div>
                <label className="ems-label" htmlFor="filter-gpa-min">
                  GPA
                </label>
                <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                  <input
                    id="filter-gpa-min"
                    aria-label="GPA tối thiểu"
                    className="ems-input"
                    style={{ textAlign: "center" }}
                    placeholder="0.0"
                    value={filters.gpaMin}
                    onChange={(e) =>
                      setFilters((f) => ({ ...f, gpaMin: e.target.value }))
                    }
                  />
                  <span style={{ color: "#7a90a8", fontSize: 12 }}>đến</span>
                  <input
                    id="filter-gpa-max"
                    aria-label="GPA tối đa"
                    className="ems-input"
                    style={{ textAlign: "center" }}
                    placeholder="4.0"
                    value={filters.gpaMax}
                    onChange={(e) =>
                      setFilters((f) => ({ ...f, gpaMax: e.target.value }))
                    }
                  />
                </div>
              </div>
              <div>
                <label className="ems-label" htmlFor="filter-attendance-min">
                  Chuyên cần tối thiểu (%)
                </label>
                <input
                  id="filter-attendance-min"
                  className="ems-input"
                  type="number"
                  min="0"
                  max="100"
                  placeholder="Ví dụ: 80"
                  value={filters.attendMin}
                  onChange={(e) =>
                    setFilters((f) => ({ ...f, attendMin: e.target.value }))
                  }
                />
              </div>
              <div style={{ display: "flex", gap: 8 }}>
                <button
                  ...
                  style={{ flex: 1 }}
                  onClick={() =>
                    setSaved((s) => [...s, "Bộ lọc " + (s.length + 1)])
                  }
                >
                  💾 Lưu bộ lọc
                </button>
                ...
                {saved.map((s) => (
                  ...
                  >
                    <button
                      ...
                      className="ems-btn ems-btn-ghost ems-btn-xs"
                      onClick={() =>
                        setFilters((f) => ({ ...f, semester: "hk1-2024" }))
                      }
                    >
                      ⊕ {s}
                    </button>
                    <button
                      type="button"
                      aria-label={`Xóa bộ lọc đã lưu ${s}`}
                      className="ems-btn ems-btn-ghost ems-btn-xs"
                      onClick={() =>
                        setSaved((filters) =>
                          filters.filter((filter) => filter !== s),
                        )
                      }
                    >
                      ✕
        ...
        <div style={{ flex: 1 }}>
          ...
          >
            ...
            {((filters.dept as string[]).length > 0 ||
                  ...
                  >
                    ...
                    <button
                      onClick={() =>
                        setFilters((f) => ({
                          ...f,
                          status: (f.status as string[]).filter(
                            (x: string) => x !== s,
                          ),
                        }))
                      }
            ...
            <span
              style={{ marginLeft: "auto", fontSize: 13, color: "#7a90a8" }}
            >
              247 bản ghi
            </span>
          </div>
          ...
          >
            ...
            <div style={{ fontSize: 14, color: "#7a90a8" }}>
              Kết quả sẽ hiện tại đây sau khi áp dụng bộ lọc
            </div>
          </div>
        </div>
      </div>
...
function ApprovalPattern() {
  const [items, setItems] = useState<ApprovalItem[]>([
    ...
    {
      id: "REQ002",
      title: "Đơn đăng ký học lại HP IT201",
      submitter: "Trần Thị Bình",
      dept: "CNTT",
      date: "11/01/2025",
      type: "Học lại",
      status: "Chờ duyệt",
    ...
    {
      id: "REQ003",
      title: "Đơn miễn giảm học phí",
      submitter: "Lê Minh Cường",
      dept: "Kinh tế",
      date: "10/01/2025",
      type: "Tài chính",
      status: "Đã duyệt",
    ...
    {
      id: "REQ004",
      title: "Đơn chuyển ngành học",
      submitter: "Phạm Thị Dung",
      dept: "Ngoại ngữ",
      date: "09/01/2025",
      type: "Chuyển ngành",
      status: "Từ chối",
    ...
    {
      id: "REQ005",
      title: "Đơn xin xét đặc cách tốt nghiệp",
      submitter: "Vũ Thị Phương",
      dept: "CNTT",
      date: "08/01/2025",
      type: "Tốt nghiệp",
      status: "Chờ duyệt",
  ...
  const approve = (id: string) => {
    ...
    if (selectedItem?.id === id)
      setSelectedItem((i) => (i ? { ...i, status: "Đã duyệt" } : null))
  }
  const reject = (id: string) => {
    setItems((prev) =>
      prev.map((i) => (i.id === id ? { ...i, status: "Từ chối" } : i)),
    )
    if (selectedItem?.id === id)
      setSelectedItem((i) => (i ? { ...i, status: "Từ chối" } : null))
  }
  ...
  return (
    <div>
      <div className="section-label">
        Approval Workflow — Duyệt đơn từ sinh viên
      </div>
      <div
        ...
        <div className="ems-card" style={{ overflow: "hidden" }}>
          <div className="ems-card-header">
            ...
            <span className="ems-badge ems-badge-blue">
              {items.filter((i) => i.status === "Chờ duyệt").length} chờ
            </span>
          </div>
          <div>
            {items.map((item) => (
              <button
                ...
                style={{
                  ...
                  cursor: "pointer",
                  background:
                    selectedItem?.id === item.id ? "#e6f4ff" : "white",
                  borderLeft: `3px solid ${
                    selectedItem?.id === item.id ? "#1677ff" : "transparent"
                  }`,
                  transition: "all 0.1s",
                  textAlign: "left",
                  width: "100%",
              ...
              >
                ...
                >
                  ...
                  <span
                    style={{
                      fontSize: 11,
                      color: priorityConfig[item.priority],
                      fontWeight: 600,
                    }}
                  >
                    ● {item.priority}
                ...
                >
                  ...
                  <span
                    className={`ems-badge ${statusConfig[item.status].cls}`}
                    style={{ fontSize: 11 }}
                  >
                    {item.status}
                  </span>
          ...
          <div className="ems-card" style={{ overflow: "hidden" }}>
            <div className="ems-card-header">
              <div>
                ...
                <div style={{ fontSize: 12, color: "#7a90a8", marginTop: 2 }}>
                  {selectedItem.id} • Nộp ngày {selectedItem.date}
                </div>
              </div>
              <span
                className={`ems-badge ${statusConfig[selectedItem.status].cls}`}
              >
                {selectedItem.status}
              </span>
            </div>
            <div className="ems-card-body">
              ...
              <div style={{ marginBottom: 16 }}>
                ...
                >
                  Kính gửi Ban Giám hiệu và Phòng Đào tạo, tôi là{" "}
                  {selectedItem.submitter}, sinh viên khoa {selectedItem.dept}.
                  Tôi xin phép {selectedItem.title.toLowerCase()} theo quy định
                  của nhà trường. Lý do: vì hoàn cảnh gia đình đặc biệt khó
                  khăn. Kính mong Ban Giám hiệu xem xét và chấp thuận đơn của
                  tôi.
                </div>
              </div>
              ...
              <div style={{ marginBottom: 16 }}>
                ...
                {["don_xin.pdf", "giay_to_bo_sung.pdf"].map((f) => (
                  <button
                    ...
                    type="button"
                    className="ems-btn ems-btn-secondary ems-btn-sm"
                    aria-label={`Tải tài liệu đính kèm ${f}`}
                    style={{
                      width: "100%",
                      justifyContent: "space-between",
                      marginBottom: 6,
              ...
              {selectedItem.status === "Chờ duyệt" && (
                <>
                  <div style={{ marginBottom: 16 }}>
                    <label className="ems-label" htmlFor="approval-note">
                      Ghi chú phê duyệt
                    </label>
                    <textarea
                      id="approval-note"
                  ...
                  >
                    <button className="ems-btn ems-btn-secondary ems-btn-md">
                      Chuyển tiếp
                    </button>
                    <button
              ...
              {selectedItem.status !== "Chờ duyệt" && (
                ...
                >
                  ...
                  <span>
                    Đơn này đã được{" "}
                    <strong>{selectedItem.status.toLowerCase()}</strong>.
                  </span>
                </div>
              )}
            </div>
              ...
              <tr>
                ...
                {DAYS.map((d) => (
                  <th
                    ...
                    style={{
                      padding: "10px 14px",
                      background: d === today ? "#e6f4ff" : "#f5f7fa",
                      borderBottom: "1px solid #d1dbe8",
                      fontSize: 13,
                      fontWeight: d === today ? 700 : 500,
                      color: d === today ? "#1677ff" : "#4a5e78",
                      textAlign: "center",
                    ...
                    return (
                      <td
                        ...
                        style={{
                          ...
                          verticalAlign: "top",
                          background:
                            day === today ? "rgba(22,119,255,0.02)" : "white",
                          borderLeft:
                            day === today
                              ? "2px solid rgba(22,119,255,0.2)"
                              : undefined,
                        }}
                      >
                        {cell && (
                          <button
                            type="button"
                            aria-label={`${cell.code}: ${cell.name}, ${cell.room}, ${cell.teacher}`}
                            style={{
                              background: cell.color,
                              border: `1px solid ${cell.color.replace("ff", "aa")}`,
                              borderRadius: 6,
                              padding: "8px 10px",
                              cursor: "pointer",
                              transition: "all 0.1s",
...
function AttendancePattern() {
  const [attendance, setAttendance] =
    useState<Record<string, "present" | "absent" | "late" | null>>({})
  const students = [
    "Nguyễn Văn An",
    "Trần Thị Bình",
    "Lê Minh Cường",
      ...
      >
          ...
          <div className="ems-card-header">
            <div>
              ...
              <div style={{ fontSize: 12, color: "#7a90a8" }}>
                Tiết 1–3 • 07:00 • Thứ Tư 08/01/2025 • H2-301
              </div>
            </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          <div className="ems-stat-card">
            ...
            <div style={{ fontSize: 28, fontWeight: 800, color: "#0f1b2d" }}>
              {students.length}
            </div>
            <div style={{ fontSize: 12, color: "#b0bec8" }}>
              Đã điểm danh: {Object.keys(attendance).length}
            </div>
          </div>
          {[
            {
            ...
            >
              <div style={{ display: "flex", justifyContent: "space-between" }}>
                ...
                <span style={{ fontSize: 24, fontWeight: 800, color: c.color }}>
                  {c.count}
                </span>
              </div>
              <div
          ...
          >
            ...
            <div
              style={{
                fontSize: 32,
                fontWeight: 800,
                color: counts.present + counts.late > 0 ? "#1677ff" : "#b0bec8",
              }}
            >
              {students.length > 0
                ? Math.round(
...
function PermissionPattern() {
  ...
  const DEFAULT_MATRIX: Record<string, Record<string, boolean>> = {
    "Quản lý sinh viên|Quản trị viên": {
      Xem: true,
      Thêm: true,
      Sửa: true,
      Xóa: true,
    },
    "Quản lý sinh viên|Trưởng khoa": {
      Xem: true,
      Thêm: false,
      Sửa: true,
      Xóa: false,
    ...
    "Nhập điểm|Trưởng khoa": {
      Xem: true,
      Nhập: false,
      Sửa: false,
      Duyệt: true,
    },
    "Nhập điểm|Quản trị viên": {
      Xem: true,
      Nhập: true,
      Sửa: true,
      Duyệt: true,
  ...
  const toggle = (module: string, role: string, perm: string) => {
    ...
    setMatrix((m) => ({
      ...m,
      [key]: { ...(m[key] || {}), [perm]: !getVal(module, role, perm) },
    }))
  }
  ...
  return (
    <div>
      <div className="section-label">
        Role Permission Matrix — Ma trận phân quyền
      </div>
      <div className="ems-card" style={{ overflow: "hidden" }}>
        <div className="ems-card-header">
          <span style={{ fontWeight: 700, fontSize: 14 }}>
            ...
            <tbody>
              {MODULES.map((mod, mi) =>
                mod.perms.map((perm, pi) => (
                  <tr
                    key={`${mod.name}-${perm}`}
                    style={{ background: mi % 2 === 0 ? "white" : "#fafbfc" }}
                  >
                    {pi === 0 && (
                      <td
                        rowSpan={mod.perms.length}
                        style={{
                          padding: "10px 16px",
                          borderBottom: "1px solid #e8eef5",
                      ...
                      return (
                        ...
                        >
                          <input
                            ...
                            disabled={isAdmin}
                            onChange={() =>
                              !isAdmin && toggle(mod.name, role, perm)
                            }
                            style={{
    ...
    <div style={{ padding: 32, maxWidth: 1200, margin: "0 auto" }}>
      <div style={{ marginBottom: 28 }}>
        ...
        <p style={{ fontSize: 14, color: "#7a90a8", margin: 0 }}>
          Luồng tương tác enterprise: bộ lọc, phê duyệt, lịch học, điểm danh,
          phân quyền
        </p>
      </div>
      <div
      ...
      >
        {TABS.map((t, i) => (
          <button
            ...
            aria-controls={`patterns-panel-${i}`}
      ...
      <div
        role="tabpanel"
        id={`patterns-panel-${activeTab}`}
        aria-labelledby={`patterns-tab-${activeTab}`}
      >
        {renderContent()}
      </div>
    </div>
